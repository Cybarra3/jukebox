(() => {
  ...
  function updateMediaSessionMetadata(item) {
    if (!mediaSessionSupported || !item) return;
    ...
    try {
      navigator.mediaSession.metadata = new MediaMetadata({
        title: item.title || "Pocket Jukebox",
        artist: item.artist || "Pocket Jukebox",
        album: item.source === "spotify" ? "Spotify" : "Pocket Jukebox",
        artwork
      });
    } catch {}
  }
  ...
  function updateMediaSessionPosition() {
    ...
    try {
      if (currentSource === "local") {
        const duration = els.localAudio.duration;
        const position = els.localAudio.currentTime;
        if (Number.isFinite(duration) && duration > 0 && Number.isFinite(position)) {
          navigator.mediaSession.setPositionState({
            duration,
            playbackRate: els.localAudio.playbackRate || 1,
            position: Math.min(Math.max(position, 0), duration)
          });
        }
      } else if (currentSource === "spotify" && spotifyState?.duration) {
        navigator.mediaSession.setPositionState({
          duration: spotifyState.duration / 1000,
          playbackRate: 1,
          position: Math.min(
            Math.max((spotifyState.position || 0) / 1000, 0),
            spotifyState.duration / 1000
          )
        });
      }
    } catch {}
  ...
  function setupMediaSessionHandlers() {
    if (!mediaSessionSupported) {
      if (els.mediaSessionStatus) {
        els.mediaSessionStatus.textContent = "Phone media controls: not supported by this browser";
      }
      return;
    }

    if (els.mediaSessionStatus) {
      els.mediaSessionStatus.textContent = "Phone media controls: enabled";
    }
    ...
    safeSetHandler("play", async () => {
      ...
      } else if (currentSource === "spotify" && spotifyPlayer) {
    ...
    safeSetHandler("previoustrack", () => previousTrack());
    ...
    safeSetHandler("seekbackward", details => {
      ...
      if (currentSource === "local" && Number.isFinite(els.localAudio.duration)) {
        els.localAudio.currentTime = Math.max(0, els.localAudio.currentTime - offset);
      } else if (currentSource === "spotify" && spotifyPlayer && spotifyState) {
        spotifyPlayer.seek(Math.max(0, spotifyState.position - offset * 1000)).catch(() => {});
      }
    });
    ...
    safeSetHandler("seekforward", details => {
      const offset = details.seekOffset || 10;
      if (currentSource === "local" && Number.isFinite(els.localAudio.duration)) {
        els.localAudio.currentTime = Math.min(els.localAudio.duration, els.localAudio.currentTime + offset);
      } else if (currentSource === "spotify" && spotifyPlayer && spotifyState) {
        spotifyPlayer.seek(Math.min(spotifyState.duration, spotifyState.position + offset * 1000)).catch(() => {});
      }
    });
    ...
    safeSetHandler("seekto", details => {
      ...
      if (currentSource === "local" && Number.isFinite(els.localAudio.duration)) {
        els.localAudio.currentTime = Math.min(Math.max(details.seekTime, 0), els.localAudio.duration);
      } else if (currentSource === "spotify" && spotifyPlayer && spotifyState) {
        spotifyPlayer.seek(
          Math.min(Math.max(details.seekTime * 1000, 0), spotifyState.duration)
        ).catch(() => {});
      }
    });
  ...
  function updateTrackDisplay(item) {
    ...
    if (item) updateMediaSessionMetadata(item);
  }
  ...
  function nextTrack() {
    if (!queue.length) {
      toast("Add songs to the queue first.");
      return;
    }
    const next = currentIndex < 0 ? 0 : (currentIndex + 1) % queue.length;
    playIndex(next);
  }
  ...
  function previousTrack() {
    ...
    // Common media-player behavior: if more than 3 seconds into a song,
    // Previous restarts the current song. Otherwise it goes to the previous song.
    if (currentSource === "local" && els.localAudio.currentTime > 3) {
      els.localAudio.currentTime = 0;
      updateMediaSessionPosition();
      return;
    }
    if (currentSource === "spotify" && spotifyState?.position > 3000 && spotifyPlayer) {
      spotifyPlayer.seek(0).catch(() => {});
      return;
    }

    const prev = currentIndex < 0 ? 0 : (currentIndex - 1 + queue.length) % queue.length;
    playIndex(prev);
  }

  els.localAudio.addEventListener("play", () => { setPlayingUI(true); updateMediaSessionPlaybackState(); });
  els.localAudio.addEventListener("pause", () => { setPlayingUI(false); updateMediaSessionPlaybackState(); });
  ...
  els.volumeBar.addEventListener("input", async () => {
    const volume = Math.min(1, Math.max(0, Number(els.volumeBar.value) || 0));
    els.localAudio.volume = volume;

    if (spotifyPlayer) {
      try { await spotifyPlayer.setVolume(volume); }
      catch {}
    }
  });

  els.volumeBar.addEventListener("change", () => {
    const pct = Math.round(Number(els.volumeBar.value) * 100);
    toast(`Jukebox volume: ${pct}%`);
  });
  ...
  function updateSpotifyStateUI(state) {
    ...
    if (sdkTrack) {
      ...
      updateMediaSessionMetadata({
        title: sdkTrack.name,
        artist: sdkTrack.artists?.map(a => a.name).join(", ") || "Spotify",
        source: "spotify",
